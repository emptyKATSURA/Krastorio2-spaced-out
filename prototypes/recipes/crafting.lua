data:extend({
  {
    type = "recipe",
    name = "kr-carbide-processing-circuit",
    enabled = false,
    energy_required = 12,
    icons = {
      { icon = "__base__/graphics/icons/processing-unit.png" },
      { icon = "__space-age__/graphics/icons/tungsten-carbide.png", scale = 0.26, shift = { -8, -8 } },
    },
    category = "electronics",
    energy_required = 6,
    auto_recycle = false,
    ingredients = {
      { type = "fluid", name = "sulfuric-acid", amount = 10 },
      { type = "item", name = "advanced-circuit", amount = 6 },
      { type = "item", name = "tungsten-carbide", amount = 3 },
    },
    results = { { type = "item", name = "processing-unit", amount = 2 } },
    allow_productivity = true,
  },
    {
    type = "recipe",
    name = "kr-bio-processing-circuit",
    enabled = false,
    energy_required = 12,
    icons = {
      { icon = "__base__/graphics/icons/processing-unit.png" },
      { icon = "__space-age__/graphics/icons/pentapod-egg.png", scale = 0.26, shift = { -8, -8 } },
    },
    category = "electronics",
    energy_required = 6,
    auto_recycle = false,
    ingredients = {
      { type = "fluid", name = "sulfuric-acid", amount = 10 },
      { type = "item", name = "advanced-circuit", amount = 6 },
      { type = "item", name = "pentapod-egg", amount = 1 },
    },
    results = { { type = "item", name = "processing-unit", amount = 2 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-antimatter-artillery-shell",
    enabled = false,
    energy_required = 15,
    ingredients = {
      { type = "item", name = "kr-imersium-plate", amount = 1 },
      { type = "item", name = "artillery-shell", amount = 1 },
      { type = "item", name = "kr-charged-antimatter-fuel-cell", amount = 1 },
      { type = "item", name = "processing-unit", amount = 1 },
    },
    results = { { type = "item", name = "kr-antimatter-artillery-shell", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-antimatter-railgun-shell",
    enabled = false,
    energy_required = 5,
    ingredients = {
      { type = "item", name = "kr-imersium-plate", amount = 1 },
      { type = "item", name = "kr-basic-railgun-shell", amount = 1 },
      { type = "item", name = "kr-charged-antimatter-fuel-cell", amount = 1 },
      { type = "item", name = "processing-unit", amount = 1 },
    },
    results = { { type = "item", name = "kr-antimatter-railgun-shell", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-antimatter-rocket",
    enabled = false,
    energy_required = 10,
    ingredients = {
      { type = "item", name = "kr-imersium-plate", amount = 2 },
      { type = "item", name = "kr-heavy-rocket", amount = 1 },
      { type = "item", name = "kr-charged-antimatter-fuel-cell", amount = 3 },
      { type = "item", name = "processing-unit", amount = 1 },
    },
    results = { { type = "item", name = "kr-antimatter-rocket", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-antimatter-turret-rocket",
    enabled = false,
    energy_required = 5,
    ingredients = {
      { type = "item", name = "kr-imersium-plate", amount = 1 },
      { type = "item", name = "kr-explosive-turret-rocket", amount = 1 },
      { type = "item", name = "kr-charged-antimatter-fuel-cell", amount = 1 },
      { type = "item", name = "processing-unit", amount = 1 },
    },
    results = { { type = "item", name = "kr-antimatter-turret-rocket", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-automation-core",
    enabled = false,
    category = "electronics",
    energy_required = 2,
    ingredients = {
      { type = "item", name = "iron-gear-wheel", amount = 4 },
      { type = "item", name = "iron-stick", amount = 4 },
      { type = "item", name = "copper-plate", amount = 6 },
    },
    results = { { type = "item", name = "kr-automation-core", amount = 2 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-basic-railgun-shell",
    enabled = false,
    energy_required = 10,
    ingredients = {
      { type = "item", name = "steel-plate", amount = 5 },
      { type = "item", name = "advanced-circuit", amount = 1 },
    },
    results = { { type = "item", name = "kr-basic-railgun-shell", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-biter-research-data",
    enabled = false,
    energy_required = 40,
    ingredients = {
      { type = "item", name = "kr-biomass", amount = 10 },
      { type = "item", name = "kr-coke", amount = 5 },
      { type = "item", name = "steel-plate", amount = 5 },
    },
    results = { { type = "item", name = "kr-biter-research-data", amount = 5 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-blank-tech-card",
    enabled = false,
    category = "electronics",
    energy_required = 2,
    ingredients = {
      { type = "item", name = "iron-plate", amount = 2 },
      { type = "item", name = "copper-cable", amount = 2 },
    },
    results = { { type = "item", name = "kr-blank-tech-card", amount = 5 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-electronic-components",
    enabled = false,
    category = "electronics",
    energy_required = 4,
    ingredients = {
      { type = "item", name = "kr-silicon", amount = 2 },
      { type = "item", name = "plastic-bar", amount = 2 },
      { type = "item", name = "kr-glass", amount = 2 },
    },
    results = { { type = "item", name = "kr-electronic-components", amount = 4 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-empty-dt-fuel-cell",
    enabled = false,
    energy_required = 10,
    ingredients = {
      { type = "item", name = "low-density-structure", amount = 2 },
      { type = "item", name = "kr-rare-metals", amount = 1 },
    },
    results = { { type = "item", name = "kr-empty-dt-fuel-cell", amount = 1 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-energy-control-unit",
    enabled = false,
    category = "electronics",
    energy_required = 10,
    ingredients = {
      { type = "item", name = "low-density-structure", amount = 6 },
      { type = "item", name = "kr-imersite-crystal", amount = 3 },
      { type = "item", name = "kr-electronic-components", amount = 9 },
      { type = "item", name = "kr-lithium-sulfur-battery", amount = 3 },
    },
    results = { { type = "item", name = "kr-energy-control-unit", amount = 1 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-explosive-railgun-shell",
    enabled = false,
    energy_required = 10,
    ingredients = {
      { type = "item", name = "kr-basic-railgun-shell", amount = 1 },
      { type = "item", name = "explosives", amount = 3 },
    },
    results = { { type = "item", name = "kr-explosive-railgun-shell", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-explosive-turret-rocket",
    enabled = false,
    energy_required = 20,
    ingredients = {
      { type = "item", name = "rocket-fuel", amount = 10 },
      { type = "item", name = "processing-unit", amount = 1 },
      { type = "item", name = "steel-plate", amount = 5 },
      { type = "item", name = "explosives", amount = 5 },
    },
    results = { { type = "item", name = "kr-explosive-turret-rocket", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-first-aid-kit",
    enabled = false,
    energy_required = 5,
    ingredients = {
      { type = "item", name = "kr-biomass", amount = 1 },
      { type = "item", name = "wood", amount = 1 },
      { type = "item", name = "iron-plate", amount = 1 },
    },
    results = { { type = "item", name = "kr-first-aid-kit", amount = 1 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-teleportation-gps-module",
    enabled = false,
    energy_required = 5,
    ingredients = {
      { type = "item", name = "processing-unit", amount = 20 },
      { type = "item", name = "kr-ai-core", amount = 10 },
    },
    results = { { type = "item", name = "kr-teleportation-gps-module", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-heavy-rocket",
    enabled = false,
    energy_required = 10,
    ingredients = {
      { type = "item", name = "explosives", amount = 10 },
      { type = "item", name = "steel-plate", amount = 5 },
      { type = "item", name = "rocket-fuel", amount = 10 },
      { type = "item", name = "plastic-bar", amount = 5 },
      { type = "item", name = "processing-unit", amount = 1 },
    },
    results = { { type = "item", name = "kr-heavy-rocket", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-heavy-rocket-launcher",
    enabled = false,
    energy_required = 60,
    ingredients = {
      { type = "item", name = "steel-plate", amount = 5 },
      { type = "item", name = "low-density-structure", amount = 5 },
      { type = "item", name = "kr-steel-gear-wheel", amount = 1 },
      { type = "item", name = "processing-unit", amount = 1 },
    },
    results = { { type = "item", name = "kr-heavy-rocket-launcher", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-imersite-rounds-magazine",
    enabled = false,
    energy_required = 20,
    ingredients = {
      { type = "item", name = "kr-imersite-crystal", amount = 2 },
      { type = "item", name = "piercing-rounds-magazine", amount = 1 },
    },
    results = { { type = "item", name = "kr-imersite-rounds-magazine", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-imersium-beam",
    enabled = false,
    energy_required = 3,
    ingredients = {
      { type = "item", name = "kr-imersium-plate", amount = 2 },
    },
    results = { { type = "item", name = "kr-imersium-beam", amount = 1 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-imersium-gear-wheel",
    enabled = false,
    energy_required = 0.5,
    ingredients = {
      { type = "item", name = "kr-imersium-plate", amount = 1 },
    },
    results = { { type = "item", name = "kr-imersium-gear-wheel", amount = 1 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-impulse-rifle",
    enabled = false,
    energy_required = 30,
    ingredients = {
      { type = "item", name = "steel-plate", amount = 5 },
      { type = "item", name = "low-density-structure", amount = 5 },
      { type = "item", name = "kr-imersite-crystal", amount = 1 },
      { type = "item", name = "kr-steel-gear-wheel", amount = 1 },
    },
    results = { { type = "item", name = "kr-impulse-rifle", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-impulse-rifle-ammo",
    enabled = false,
    energy_required = 5,
    ingredients = {
      { type = "item", name = "kr-lithium-sulfur-battery", amount = 1 },
      { type = "item", name = "kr-imersite-crystal", amount = 3 },
      { type = "item", name = "plastic-bar", amount = 1 },
    },
    results = { { type = "item", name = "kr-impulse-rifle-ammo", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-inserter-parts",
    enabled = true,
    energy_required = 1,
    ingredients = {
      { type = "item", name = "iron-stick", amount = 2 },
      { type = "item", name = "iron-gear-wheel", amount = 2 },
    },
    results = { { type = "item", name = "kr-inserter-parts", amount = 1 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-iron-beam",
    enabled = true,
    energy_required = 2,
    ingredients = {
      { type = "item", name = "iron-plate", amount = 2 },
    },
    results = { { type = "item", name = "kr-iron-beam", amount = 1 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-matter-stabilizer",
    enabled = false,
    energy_required = 5,
    ingredients = {
      { type = "item", name = "kr-imersium-plate", amount = 6 },
      { type = "item", name = "kr-energy-control-unit", amount = 3 },
      { type = "item", name = "processing-unit", amount = 3 },
    },
    results = { { type = "item", name = "kr-matter-stabilizer", amount = 1 } },
    allow_as_intermediate = false,
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-nuclear-artillery-shell",
    enabled = false,
    energy_required = 15,
    ingredients = {
      { type = "item", name = "processing-unit", amount = 10 },
      { type = "item", name = "steel-plate", amount = 10 },
      { type = "item", name = "artillery-shell", amount = 1 },
      { type = "item", name = "uranium-235", amount = 10 },
    },
    results = { { type = "item", name = "kr-nuclear-artillery-shell", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-nuclear-turret-rocket",
    enabled = false,
    energy_required = 60,
    ingredients = {
      { type = "item", name = "kr-explosive-turret-rocket", amount = 1 },
      { type = "item", name = "steel-plate", amount = 5 },
      { type = "item", name = "uranium-235", amount = 10 },
      { type = "item", name = "processing-unit", amount = 1 },
    },
    results = { { type = "item", name = "kr-nuclear-turret-rocket", amount = 1 } },
  },
  {
    type = "recipe",
    name = "kr-steel-beam",
    enabled = false,
    energy_required = 2,
    ingredients = {
      { type = "item", name = "steel-plate", amount = 2 },
    },
    results = { { type = "item", name = "kr-steel-beam", amount = 1 } },
    allow_productivity = true,
  },
  {
    type = "recipe",
    name = "kr-steel-gear-wheel",
    enabled = false,
    energy_required = 0.5,
    ingredients = {
      { type = "item", name = "steel-plate", amount = 1 },
    },
    results = { { type = "item", name = "kr-steel-gear-wheel", amount = 1 } },
    allow_productivity = true,
  },
})
